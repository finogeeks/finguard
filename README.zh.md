...
本仓库是安装文档、本机实验脚本、Helm 与运维技能的 **公开入口**。不含引擎源码。**签名 + SBOM：** 首次签名发布之后，按 [`docs/supply-chain.zh.md`](docs/supply-chain.zh.md) 校验。该仪式之前的标签仍是未签名的。在 `cosign verify` 通过之前，不要把 GHCR 当作 GA 供应链证明。
