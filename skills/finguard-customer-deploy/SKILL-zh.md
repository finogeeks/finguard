...
- [ ] 若做凭证托管：叠加 Vault — docs/vault-custody.zh.md（`FINGUARD_VAULT_ADDR` + `FINGUARD_INJECT_HEADER`；不要环境变量注入）
...
（`oidc.issuer` / `oidc.audience` / `oidc.jwksUrl`）。凭证托管：
`docs/vault-custody.zh.md`。图表不模板化 Vault 注入开关。
